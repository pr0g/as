...
TEST_CASE("quat_multiplication", "[as_quat]")
{
  ...
  const quat_t quat_b = quat::axis_angle(vec3_t::axis_y(), deg_to_rad(180.0_r));
...
{
  {
...

    {
      ...
      const vec3_t result_vec = quat::rotate(result_quat, vec3_t::axis_x());
...
TEST_CASE("quat_from_mat3", "[as_quat]")
{
...

  {
    ...
    vec3_t result_matrix = affine::transform_dir(affine_t(axis_angle_mat), vec);
