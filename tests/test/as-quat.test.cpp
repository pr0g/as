...
{
  const quat_t quat_a =
    quat_rotation_axis(vec3_t::axis_x(), deg_to_rad(90.0_r));
  const quat_t quat_b =
    quat_rotation_axis(vec3_t::axis_y(), deg_to_rad(180.0_r));
...
TEST_CASE("quat_rotate_vec", "[as_quat]")
{
  ...
  {
    const quat_t quat_x =
      quat_rotation_axis(vec3_t::axis_x(), deg_to_rad(270.0_r));
    const quat_t quat_z =
      quat_rotation_axis(vec3_t::axis_z(), deg_to_rad(-90.0_r));
    const quat_t quat_zx = quat_z * quat_x;

    const vec3_t result = quat_rotate(quat_zx, vec3_t::axis_z());
...
{
  {
    const quat_t quat_y = quat_rotation_axis(vec3_t::axis_z(), 0.0_r);
    const quat_t quat_x =
      quat_rotation_axis(vec3_t::axis_z(), deg_to_rad(90.0_r));

    {
      const quat_t result_quat = quat_slerp(quat_x, quat_y, 0.5_r);
      const vec3_t result_vec = quat_rotate(result_quat, vec3_t::axis_x());
    ...
    {
      const quat_t result_quat = quat_slerp(quat_x, quat_y, 0.333333_r);
      const vec3_t result_vec = quat_rotate(result_quat, vec3_t::axis_x());
    ...
    {
      const quat_t result_quat = quat_slerp(quat_x, quat_y, 0.666666_r);
      const vec3_t result_vec = quat_rotate(result_quat, vec3_t::axis_x());
