...
TEST_CASE("affine_constructor_position_rotation", "[as_affine]")
{
  ...
  CHECK(mat::equal(rotation, affine.rotation));
}
...
TEST_CASE("affine_transform_dir", "[as_affine]")
{
  const affine_t affine(
    mat3::rotation_y(deg_to_rad(90.0_r)), vec3_t(5.0_r, 0.0_r, 0.0_r));
...
{
  const affine_t affine =
    affine_t(mat3::rotation_y(deg_to_rad(90.0_r)), vec3_t(5.0_r, 0.0_r, 0.0_r));
...
TEST_CASE("affine_from_mat3_vec3", "[as_affine]")
{
  ...
  affine_t affine;
  affine = affine::from_mat3_vec3(mat3, vec3);
