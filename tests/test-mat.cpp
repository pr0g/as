...
TEST_CASE("mat3_axis_angle", "[as-mat]")
{
    ...
    as::mat3_t axis_angle = as::mat3::axis_angle(as::vec3::axis_x(), as::deg_to_rad(90.0f));
    ...
    CHECK_THAT(
        span(result_reference),
        ElementsAreSubscript(result, 3).margin(std::numeric_limits<float>::epsilon())
    );
}
